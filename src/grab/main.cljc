(ns grab.main
  "Kotodama WASM entrypoint for the Grab clean-room actor (L4) — Clojure port.
  ...
  facts (`grab.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "User"       :plural "users"       :id-prefix "grab_use"
    :fields [:phone :displayName :kycStatus :walletId] :required [:phone :displayName]
    :coerce {}                       :refs {:walletId "Wallet"}}
   {:entity "Wallet"     :plural "wallets"     :id-prefix "grab_wal"
    :fields [:userId :currency :balance :status]      :required [:currency :balance]
    :coerce {:balance :float}        :refs {:userId "User"}}
   {:entity "Merchant"   :plural "merchants"   :id-prefix "grab_mer"
    :fields [:name :category :rating]                 :required [:name :category]
    :coerce {:rating :float}         :refs {}}
   {:entity "RideOrder"  :plural "rideorders"  :id-prefix "grab_rid"
    :fields [:userId :origin :destination :fare :status] :required [:origin :destination]
    :coerce {:fare :float}           :refs {:userId "User"}}
   {:entity "FoodOrder"  :plural "foodorders"  :id-prefix "grab_foo"
    :fields [:userId :merchantId :total :status]      :required [:total :status]
    :coerce {:total :float}          :refs {:userId "User" :merchantId "Merchant"}}
   {:entity "Payment"    :plural "payments"    :id-prefix "grab_pay"
    :fields [:userId :amount :currency :method :status] :required [:amount :currency]
    :coerce {:amount :float}         :refs {:userId "User"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"grab.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "grab-compat" :tier tier :entities entities} 200])
